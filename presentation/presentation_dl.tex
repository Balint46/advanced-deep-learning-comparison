...
\author[B.~Kir\'{a}ly \& Sz.~Hoffmann]{B\'{a}lint Kir\'{a}ly \& Szabolcs Hoffmann}
...
\begin{frame}{Experimental Setup}
  ...
  \begin{columns}
    \column{0.55\textwidth}
      ...
      \begin{tabular}{@{}lllr@{}}
        \toprule
        Model      & Type        & Pretr. & Params \\
        \midrule
        small\_cnn & CNN         & No  & 0.14\,M \\
        ResNet-18  & CNN         & Yes & 11.2\,M \\
        ViT-Tiny   & Transformer & Yes & 5.5\,M \\
        \bottomrule
      \end{tabular}
    ...
    \column{0.42\textwidth}
      ...
      \textbf{Protocol:}
      \begin{itemize}
        \item Early stopping (patience~5, max~20 epochs); best-weight restoration
        \item 3 seeds (42\,/\,123\,/\,7): error bars confirm the ranking is real
        \item All comparisons: full fine-tuning + linear head --- differences reflect the \emph{model}, not the recipe
      \end{itemize}
...
\begin{frame}{Training Dynamics}
  ...
  \begin{itemize}
    \item ResNet-18 converges fastest and plateaus at the lowest loss
    \item ViT-Tiny is slowest and remains at the highest loss throughout
    \item small\_cnn and ViT-Tiny are still improving at epoch~20 ---
          the budget, not convergence, stopped them
  \end{itemize}
...
\begin{frame}{Training Strategy Matters}
  ...
  \begin{itemize}
    ...
    \item ViT-Tiny:\phantom{x} full 71.4\,\% \;/\; freeze$\!\to\!$unfreeze 79.7\,\% \;/\; head-only 40.0\,\%
...
\begin{frame}{Key Finding: ViT's Gap Is a Trainability Artifact}
  ...
  \begin{itemize}
    ...
    \item A gentler lr closes $\approx$8 percent, nearly matching the CNNs
...
\begin{frame}{Classifier Head Depth}
  ...
  \begin{itemize}
    \item Linear / one-hidden / two-hidden differ by $\lesssim$1 percent for small CNN and ResNet-18
...
\begin{frame}{Scaling to a Harder Dataset: CIFAR-100}
  ...
  \begin{itemize}
    \item All drop $\approx$25--30 percent; \textbf{ordering is preserved}
...
\begin{frame}{Use of Artificial Intelligence}
  ...
  \begin{itemize}
    \item \textbf{Code and pipeline:} the training and evaluation pipeline was
          built iteratively with AI assistance; we reviewed every change, diagnosed
          failures, and directed each round of redesign
    \item \textbf{Presentation:} these slides were generated by AI based on our
          content and requirements
  \end{itemize}

  \bigskip
  \textit{We designed the experiments, identified the under-training issue,
  directed the full rerun, and verified every result.
  All interpretation is our own.}
